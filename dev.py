...
def apply_indicators(dataset):
    ...
    indicator_list = [
        ...
        (indicators.ema, 60, 'ema60v', 'volume'),
...
def strat(api: provider.BitvavoRestClient, market: str):
    ...
    sell_signal = \
        ...
        (btc != 0) & (history * 0.95 > data.iloc[-1].close) # stoploss
...
def test():
    ...
    api = provider.TestClient(api_key=provider.settings()['key'], api_secret=provider.settings()['secret'], balance={'EUR': 1000})
    api.set_data(data)
